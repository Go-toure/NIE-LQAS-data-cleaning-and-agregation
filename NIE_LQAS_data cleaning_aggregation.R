...
process_lqas_data <- function(input_file) {
  ...
  output_file <- "C:/Users/TOURE/Mes documents/REPOSITORIES/LQAS_raw_data/LQAS_level1/272.csv"
  ...
  F1<-F1 |> 
    mutate(vaccine.type = case_when(
      District %in% districts & response == "NIE-2025-04-01_nOPV_NIDs" & roundNumber =="Rnd1"  ~ "nOPV2 & bOPV",
      TRUE ~ vaccine.type
    )) |> 
    select(
